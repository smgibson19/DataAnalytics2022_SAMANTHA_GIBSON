...
# k means is an unsupervised learning technique to classify unlabeled data 
# by grouping them by features, rather than pre-defined categories
km3 <- kmeans(xy, 3) # three clusters
plot(xy, col = km3$cluster)
cex = 2.0
points(km3$centers, pch=3)

# same data points grouped into 4 clusters instead of 3
km4 <- kmeans(xy, 4) # four clusters
cex = 1.0
